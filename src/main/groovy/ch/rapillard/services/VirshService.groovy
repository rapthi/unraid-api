...
class VirshService {
    ...
    List<VirtualMachine>  getAll() {
        def fileContent = new File('./virsh-list.txt')
        extractVirtualMachinesFromConsoleOutput(fileContent.text)
    }
     ...
     */
    private String[] dropHeader(String[] lines) {
     ...
     * @return A List of VirtualMachine
     */
    private List<VirtualMachine> extractVirtualMachinesFromConsoleOutput(String consoleOutput) {
     ...
     */
    private VirtualMachineState extractStatus(List<String> words) {
     ...
     */
    private int extractIdentifier(String word) {
